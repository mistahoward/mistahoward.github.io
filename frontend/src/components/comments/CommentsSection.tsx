...
export const CommentsSection = ({ blogSlug }: CommentsSectionProps) => {
	...
	useEffect(() => {
		...
		const handleUserActivity = () => {
			// Only update state if we're not already in user activity mode
			if (!userActivity) {
			...
			}, 10000); // Resume polling after 10 seconds of inactivity (increased from 5)
		};

		// Listen for user interactions, but be more selective
		const events = ["mousedown", "keypress", "scroll", "touchstart"]; // Removed mousemove as it's too frequent
	...
	}, [blogSlug, user, userActivity, isPolling, lastCommentCount, fetchComments]);

	const sortComments = useCallback(
		(commentsToSort: Comment[]): Comment[] => {
	...
	const voteComment = useCallback(
		async (commentId: string, voteType: number) => {
			try {
				const token = await getIdToken();
				if (!token) throw new Error("Not authenticated");
