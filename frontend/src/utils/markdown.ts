...
export const markdownToHtml = (markdown: string): string => {
	return measureTime("markdownToHtml", () => {
		...
		const html = markdown
			// Handle horizontal rules first (before other transformations)
			.replace(/^(\s*[-*_]{3,}\s*)$/gm, "<hr />")
			// Headers
			.replace(/^### (.*$)/gim, "<h3>$1</h3>")
			.replace(/^## (.*$)/gim, "<h2>$1</h2>")
			.replace(/^# (.*$)/gim, "<h1>$1</h1>")
			// Bold text - use non-greedy matching and ensure it doesn't cross line boundaries
			.replace(/\*\*([^*\n]+)\*\*/g, "<strong>$1</strong>")
			// Italic text - use non-greedy matching and ensure it doesn't cross line boundaries
			.replace(/\*([^*\n]+)\*/g, "<em>$1</em>")
			// Images
		...
		const sanitizedHtml = DOMPurify.sanitize(html, {
			ALLOWED_TAGS: [
				...
				"a", "img", "ul", "ol", "li", "blockquote", "code", "pre", "hr"
			],
