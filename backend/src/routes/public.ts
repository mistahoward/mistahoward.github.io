...
router.get('/api/blog/:slug', async (request: Request, env: Env, ctx: any) => {
	try {
		...
		if (!slug) {
			return errorResponse('Slug is required', env, 400, request);
		}
		const db = drizzle(env.DB);
		...
		if (!post.length) {
			return errorResponse('Post not found', env, 404, request);
		}
		...
		return successResponse(postWithTags, env, 200, request);
	} catch (error) {
		return errorResponse('Failed to fetch blog post', env, 500, request);
	}
});
...
router.post('/api/testimonials', async (request: Request, env: Env) => {
	try {
		...
		if (!clientName || !content || !relationship || !token) return errorResponse('Missing required fields', env, 400, request);

		const db = drizzle(env.DB);
		const inviteRows = await db.select().from(testimonialInvites).where(eq(testimonialInvites.token, token));
		const invite = inviteRows[0];
		if (!invite) return errorResponse('Invalid or expired invite token', env, 403, request);
		if (invite.used) return errorResponse('This invite link has already been used', env, 403, request);
		if (invite.expiresAt && new Date(invite.expiresAt) < new Date()) return errorResponse('This invite link has expired', env, 403, request);
		...
		return successResponse({ message: 'Testimonial submitted for review.' }, env, 201, request);
	} catch (error) {
		return errorResponse('Failed to submit testimonial', env, 500, request);
	}
});
...
// GET /pets - Get all active pets and memorial pets
router.get('/api/pets', async (request: Request, env: Env) => {
	try {
		const db = drizzle(env.DB);
		const allPets = await db.select().from(pets)
			.where(sql`${pets.isActive} = 1 OR ${pets.isMemorial} = 1`)
...
router.get('/api/testimonials/validate-token', async (request: Request, env: Env) => {
	try {
		...
		if (!token) return errorResponse('Token is required', env, 400, request);
		const db = drizzle(env.DB);
		const inviteRows = await db.select().from(testimonialInvites).where(eq(testimonialInvites.token, token));
		const invite = inviteRows[0];
		if (!invite) return successResponse({ valid: false, reason: 'Invalid or expired invite token.' }, env, 200, request);
		if (invite.used) return successResponse({ valid: false, reason: 'This invite link has already been used.' }, env, 200, request);
		if (invite.expiresAt && new Date(invite.expiresAt) < new Date()) return successResponse({ valid: false, reason: 'This invite link has expired.' }, env, 200, request);
		return successResponse({ valid: true }, env, 200, request);
	} catch (error) {
		return errorResponse('Failed to validate invite token', env, 500, request);
	}
});
